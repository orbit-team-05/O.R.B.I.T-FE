...
export function OwnerTransactionsPage() {
    ...
    const loadTransactions = useCallback(async (nextFilters, page = 0, nextSortKey = sortKey) => {
        if (!farmId) return;
        try {
            setLoading(true);
            setError("");
            setReport(await getOwnerFarmTransactionReport(farmId, {
                ...nextFilters,
                page,
                size: 20,
                sort: REPORT_SORTS[nextSortKey] || REPORT_SORTS.createdDesc,
            }));
        } catch (err) {
    ...
    function handleSortChange(nextSortKey) {
        ...
        void loadTransactions(filters, 0, nextSortKey);
    }
        ...
        <div className="min-h-full space-y-6">
            ...
            <main className="space-y-6 px-6 pb-6">
                ...
                <section className="overflow-hidden rounded-xl border border-slate-200 bg-white">
                    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-5 py-4"><div><h2 className="font-semibold text-slate-900">Lịch sử giao dịch</h2><p className="mt-1 text-xs text-slate-500">Chọn một dòng để xem chi tiết nghiệp vụ.</p></div><div className="flex items-center gap-3"><span className="text-xs text-slate-500">{report?.totalElements ?? 0} bản ghi</span><SortSelect value={sortKey} onChange={handleSortChange} options={[{ value: "createdDesc", label: "Mới nhất trước" }, { value: "createdAsc", label: "Cũ nhất trước" }, { value: "quantityDesc", label: "Khối lượng giảm dần" }, { value: "amountDesc", label: "Giá trị giảm dần" }, { value: "status", label: "Theo trạng thái" }]} /></div></div>
                    ...
                    <Pagination page={currentPage} totalPages={totalPages} totalElements={report?.totalElements ?? 0} pageSize={report?.size ?? 20} loading={loading} onPageChange={(nextPage) => void loadTransactions(filters, nextPage)} />
                </section>
            </main>

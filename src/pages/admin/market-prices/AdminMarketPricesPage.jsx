...
function AdminMarketPricesHeader({
    onRunCrawl = () => {},
    crawling = false,
}) {
    return (
        <header className="flex items-start justify-between gap-4">
            ...
            <button
                ...
                disabled={crawling}
                onClick={onRunCrawl}
                className="inline-flex h-9 items-center justify-center rounded-lg bg-[#006948] px-4 text-sm font-semibold text-white transition hover:bg-[#00583d] disabled:cursor-not-allowed disabled:opacity-50"
            >
                {crawling ? (
                    <>
                        <svg
...
export function AdminMarketPricesPage() {
    ...
    async function handleRunCrawl() {
        try {
            ...
            if (res?.success) {
                toast.success(
                    `Crawl thành công! Đã lưu mới ${res?.count || 0} dữ liệu giá.`,
                );
            ...
            toast.error(
                res?.error || "Không thể chạy crawler.",
            );
        } catch (err) {
            console.error(err);

            toast.error(
                err?.message || "Có lỗi xảy ra khi crawl dữ liệu.",
            );
        }
    }
            ...
            <section className="space-y-5">
                ...
                <div className="rounded-xl border border-red-200 bg-red-50 p-5">
                    <p className="text-sm font-medium text-red-700">
                        {error || "Không thể tải dữ liệu giá thị trường."}
                    </p>
